...
public class SliceViewManagerWrapper extends SliceViewManagerBase {
    ...
    public final boolean isAuthoritySuspended(String authority) {
        String orDefault = this.mCachedAuthorities.getOrDefault(authority, null);
        if (orDefault == null) {
            ...
            this.mCachedAuthorities.put(authority, orDefault);
        }
        return isPackageSuspended(orDefault);
    }

    public final boolean isPackageSuspended(String pkg) {
        Boolean orDefault = this.mCachedSuspendFlags.getOrDefault(pkg, null);
        if (orDefault == null) {
            try {
                Boolean valueOf = Boolean.valueOf((this.mContext.getPackageManager().getApplicationInfo(pkg, 0).flags & IntMath.MAX_SIGNED_POWER_OF_TWO) != 0);
