        ...
        public ColorStateList getTintListForDrawableRes(Context context, int i) {
            if (i == R.drawable.abc_edit_text_material) {
                ...
                if (i == R.drawable.abc_spinner_mtrl_am_alpha || i == R.drawable.abc_spinner_textfield_background_material) {
                    ThreadLocal<TypedValue> threadLocal3 = AppCompatResources.TL_TYPED_VALUE;
                    return context.getColorStateList(R.color.abc_tint_spinner);
                } else if (arrayContains(this.TINT_COLOR_CONTROL_NORMAL, i)) {
                    return ThemeUtils.getThemeAttrColorStateList(context, R.attr.colorControlNormal);
                } else {
                    if (arrayContains(this.TINT_COLOR_CONTROL_STATE_LIST, i)) {
                        ThreadLocal<TypedValue> threadLocal4 = AppCompatResources.TL_TYPED_VALUE;
                        return context.getColorStateList(R.color.abc_tint_default);
                    } else if (arrayContains(this.TINT_CHECKABLE_BUTTON_LIST, i)) {
                        ThreadLocal<TypedValue> threadLocal5 = AppCompatResources.TL_TYPED_VALUE;
                        return context.getColorStateList(R.color.abc_tint_btn_checkable);
                    } else if (i != R.drawable.abc_seekbar_thumb_material) {
                        return null;
                    } else {
                        ThreadLocal<TypedValue> threadLocal6 = AppCompatResources.TL_TYPED_VALUE;
