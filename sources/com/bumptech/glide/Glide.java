...
public class Glide implements ComponentCallbacks2 {
    ...
    public Glide(Context context, Engine engine, MemoryCache memoryCache, BitmapPool bitmapPool, ArrayPool arrayPool, RequestManagerRetriever requestManagerRetriever, ConnectivityMonitorFactory connectivityMonitorFactory, int logLevel, RequestOptions defaultRequestOptions, Map<Class<?>, TransitionOptions<?, ?>> defaultTransitionOptions, List<RequestListener<Object>> defaultRequestListeners) {
        ...
        DecodeFormat decodeFormat = (DecodeFormat) defaultRequestOptions.options.get(Downsampler.DECODE_FORMAT);
        ...
        this.glideContext = new GlideContext(context, arrayPool, registry, new ImageViewTargetFactory(), defaultRequestOptions, defaultTransitionOptions, defaultRequestListeners, engine, logLevel);
    }
    ...
    public static void initializeGlide(Context context, GlideBuilder builder) {
        ...
        try {
            ...
            builder.requestManagerFactory = null;
            for (GlideModule glideModule3 : arrayList) {
                glideModule3.applyOptions(applicationContext, builder);
            }
            if (builder.sourceExecutor == null) {
                int calculateBestThreadCount = GlideExecutor.calculateBestThreadCount();
                builder.sourceExecutor = new GlideExecutor(new ThreadPoolExecutor(calculateBestThreadCount, calculateBestThreadCount, 0L, TimeUnit.MILLISECONDS, new PriorityBlockingQueue(), new GlideExecutor.DefaultThreadFactory("source", GlideExecutor.UncaughtThrowableStrategy.DEFAULT, false)));
            }
            if (builder.diskCacheExecutor == null) {
                builder.diskCacheExecutor = new GlideExecutor(new ThreadPoolExecutor(1, 1, 0L, TimeUnit.MILLISECONDS, new PriorityBlockingQueue(), new GlideExecutor.DefaultThreadFactory("disk-cache", GlideExecutor.UncaughtThrowableStrategy.DEFAULT, true)));
            }
            if (builder.animationExecutor == null) {
                builder.animationExecutor = GlideExecutor.newAnimationExecutor();
            }
            if (builder.memorySizeCalculator == null) {
                builder.memorySizeCalculator = new MemorySizeCalculator(new MemorySizeCalculator.Builder(applicationContext));
            }
            if (builder.connectivityMonitorFactory == null) {
                builder.connectivityMonitorFactory = new DefaultConnectivityMonitorFactory();
            }
            if (builder.bitmapPool == null) {
                int i = builder.memorySizeCalculator.bitmapPoolSize;
                if (i > 0) {
                    builder.bitmapPool = new LruBitmapPool(i);
                } else {
                    builder.bitmapPool = new BitmapPoolAdapter();
                }
            }
            if (builder.arrayPool == null) {
                builder.arrayPool = new LruArrayPool(builder.memorySizeCalculator.arrayPoolSize);
            }
            if (builder.memoryCache == null) {
                builder.memoryCache = new LruResourceCache(builder.memorySizeCalculator.memoryCacheSize);
            }
            if (builder.diskCacheFactory == null) {
                builder.diskCacheFactory = new InternalCacheDiskCacheFactory(applicationContext);
            }
            if (builder.engine == null) {
                builder.engine = new Engine(builder.memoryCache, builder.diskCacheFactory, builder.diskCacheExecutor, builder.sourceExecutor, new GlideExecutor(new ThreadPoolExecutor(0, Integer.MAX_VALUE, GlideExecutor.KEEP_ALIVE_TIME_MS, TimeUnit.MILLISECONDS, new SynchronousQueue(), new GlideExecutor.DefaultThreadFactory("source-unlimited", GlideExecutor.UncaughtThrowableStrategy.DEFAULT, false))), GlideExecutor.newAnimationExecutor(), null, null, null, null, null, null, false);
            }
            List<RequestListener<Object>> list = builder.defaultRequestListeners;
            if (list == null) {
                builder.defaultRequestListeners = Collections.emptyList();
            } else {
                builder.defaultRequestListeners = Collections.unmodifiableList(list);
            }
            RequestManagerRetriever requestManagerRetriever = new RequestManagerRetriever(builder.requestManagerFactory);
            Engine engine = builder.engine;
            MemoryCache memoryCache = builder.memoryCache;
            BitmapPool bitmapPool = builder.bitmapPool;
            ArrayPool arrayPool = builder.arrayPool;
            ConnectivityMonitorFactory connectivityMonitorFactory = builder.connectivityMonitorFactory;
            RequestOptions requestOptions = builder.defaultRequestOptions;
            requestOptions.isLocked = true;
            Glide glide2 = new Glide(applicationContext, engine, memoryCache, bitmapPool, arrayPool, requestManagerRetriever, connectivityMonitorFactory, 4, requestOptions, builder.defaultTransitionOptions, builder.defaultRequestListeners);
    ...
    public static void throwIncorrectGlideModule(Exception e) {
