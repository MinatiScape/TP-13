...
public final class DrawableToBitmapConverter {
    ...
    public static Resource<Bitmap> convert(BitmapPool bitmapPool, Drawable drawable, int width, int height) {
        ...
        if (current instanceof BitmapDrawable) {
            ...
            if (width != Integer.MIN_VALUE || current.getIntrinsicWidth() > 0) {
                if (height != Integer.MIN_VALUE || current.getIntrinsicHeight() > 0) {
                    if (current.getIntrinsicWidth() > 0) {
                        width = current.getIntrinsicWidth();
                    }
                    if (current.getIntrinsicHeight() > 0) {
                        height = current.getIntrinsicHeight();
                    }
