...
public class LruBitmapPool implements BitmapPool {
    ...
    public final synchronized Bitmap getDirtyOrNull(int width, int height, Bitmap.Config config) {
        Bitmap bitmap;
        if (config != Bitmap.Config.HARDWARE) {
            bitmap = ((SizeConfigStrategy) this.strategy).get(width, height, config != null ? config : DEFAULT_CONFIG);
            if (bitmap == null) {
                if (Log.isLoggable("LruBitmapPool", 3)) {
                    Objects.requireNonNull((SizeConfigStrategy) this.strategy);
                    String valueOf = String.valueOf(SizeConfigStrategy.getBitmapString(Util.getBitmapByteSize(width, height, config), config));
            ...
            if (Log.isLoggable("LruBitmapPool", 2)) {
                Objects.requireNonNull((SizeConfigStrategy) this.strategy);
                String valueOf2 = String.valueOf(SizeConfigStrategy.getBitmapString(Util.getBitmapByteSize(width, height, config), config));
    ...
    public synchronized void setSizeMultiplier(float sizeMultiplier) {
        long round = Math.round(((float) this.initialMaxSize) * sizeMultiplier);
        this.maxSize = round;
        trimToSize(round);
    }
    ...
    public final synchronized void trimToSize(long size) {
        while (this.currentSize > size) {
            ...
            long j = this.currentSize;
            Objects.requireNonNull((SizeConfigStrategy) this.strategy);
            this.currentSize = j - Util.getBitmapByteSize(removeLast);
            this.evictions++;
            if (Log.isLoggable("LruBitmapPool", 3)) {
