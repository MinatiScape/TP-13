...
public class Engine implements EngineJobListener, MemoryCache.ResourceRemovedListener, EngineResource.ResourceListener {
    ...
    public static class EngineJobFactory {
        ...
        public EngineJobFactory(GlideExecutor diskCacheExecutor, GlideExecutor sourceExecutor, GlideExecutor sourceUnlimitedExecutor, GlideExecutor animationExecutor, EngineJobListener listener) {
        ...
        public static void shutdownAndAwaitTermination(ExecutorService pool) {
            ((GlideExecutor) pool).delegate.shutdown();
            try {
                TimeUnit timeUnit = TimeUnit.SECONDS;
                GlideExecutor glideExecutor = (GlideExecutor) pool;
    ...
    public Engine(MemoryCache cache, DiskCache.Factory diskCacheFactory, GlideExecutor diskCacheExecutor, GlideExecutor sourceExecutor, GlideExecutor sourceUnlimitedExecutor, GlideExecutor animationExecutor, Jobs jobs, EngineKeyFactory keyFactory, ActiveResources activeResources, EngineJobFactory engineJobFactory, DecodeJobFactory decodeJobFactory, ResourceRecycler resourceRecycler, boolean isActiveResourceRetentionAllowed) {
        ...
        this.engineJobFactory = engineJobFactory == null ? new EngineJobFactory(diskCacheExecutor, sourceExecutor, sourceUnlimitedExecutor, animationExecutor, this) : engineJobFactory;
        ...
        ((LruResourceCache) cache).listener = this;
    }
    ...
    public static void logWithTimeAndKey(String log, long startTime, Key key) {
        ...
        StringBuilder sb = new StringBuilder(valueOf.length() + log.length() + 37);
        sb.append(log);
        sb.append(" in ");
    ...
    public void onResourceReleased(Key cacheKey, EngineResource<?> resource) {
        Util.assertMainThread();
        ActiveResources.ResourceWeakReference remove = this.activeResources.activeEngineResources.remove(cacheKey);
        if (remove != null) {
            remove.resource = null;
            remove.clear();
        }
        if (resource.isCacheable) {
            ((LruResourceCache) this.cache).put(cacheKey, resource);
        } else {
