...
public class StartRotationDialogFragment extends DialogFragment {
    ...
    public Dialog onCreateDialog(Bundle bundle) {
        ...
        CornerTreatment systemFeatureChecker = InjectorProvider.getInjector().getSystemFeatureChecker();
        Context context = getContext();
        Objects.requireNonNull(systemFeatureChecker);
        if (context.getPackageManager().hasSystemFeature("android.hardware.telephony")) {
            checkBox.setChecked(this.mIsWifiOnlyChecked);
            checkBox.setOnClickListener(new BottomActionBar$$ExternalSyntheticLambda1(this, checkBox));
        } else {
